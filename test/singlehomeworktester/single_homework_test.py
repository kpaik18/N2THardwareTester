...
from configuration.configuration import IConfigurationParser, ConfigurationParser
from n2tconfig import PROJECT_PATH
from singlehomeworktester.single_tester import ISingleHomeworkTester, SingleHomeworkTester


def test_valid_homework() -> None:
    test_archive_folder_path = os.path.join(PROJECT_PATH, 'test', 'singlehomeworktester', 'homeworkarchives')
    valid_archive_path = os.path.join(test_archive_folder_path, 'valid.zip')

    config_parser: IConfigurationParser = ConfigurationParser()
    config = config_parser.parse_configuration_file(os.path.join(test_archive_folder_path, 'config', 'hw1.yml'))
